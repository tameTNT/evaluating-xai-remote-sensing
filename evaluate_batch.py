...
if __name__ == "__main__":
    ...
    for c in tqdm(range(dataset.N_CLASSES), ncols=110, desc="xAI per class"):
        ...
        e = functools.reduce(lambda x, y: x | y, explainers_for_c)

        helpers.plotting.visualise_importance(e.input, e.ranked_explanation,
                                              alpha=.2, with_colorbar=False)
        plt.title(f"Class {c:02}, explainer idx 0")
        plt.show()
